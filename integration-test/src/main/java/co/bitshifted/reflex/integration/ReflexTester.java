...
public class ReflexTester {
    ...
    public static void main(String... args) throws Exception {
        ...
        var maxLen = testResults.stream().max(Comparator.comparingInt(tr -> tr.name().length())).get().name().length();

       testResults.forEach(tr -> System.out.println(tr.displayResult(maxLen + Constants.OUTPUT_PADDING)));

       int finalTestResult = testResults.stream()
               .filter(tr -> tr.result() == Constants.TEST_RESULT_FAIL).findFirst()
